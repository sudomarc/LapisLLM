"""Textual-compatible full-screen training observability console."""
...
class TrainingConsoleRunner:
    ...
    def run_process(self) -> int:
        ...
        self.process = subprocess.Popen(
            self.command,
            cwd=Path(self.config).resolve().parents[1] if Path(self.config).is_absolute() else Path.cwd(),
            text=True,
    ...
    def launch(self) -> None:
        ...
        class TrainingApp(App[None]):
            ...
            def render_sidebar(self, state: TrainingState) -> str:
                return "\n".join([
                    "LAPISLLM", "TRAINING", "", f"STATUS  {state.status}",
                    f"RUN     {state.run_id}", "", "CONTROLS",
                    "r  refresh", "p  pause/resume", "q  quit",
                ])
            ...
            def render_view(self, state: TrainingState) -> str:
                ...
                lines = [
                    ...
                    f"LOSS             {loss}", f"BEST LOSS        {best}",
                    f"TREND            {_trend(state.loss_history)}", f"THROUGHPUT       {speed}",
                    f"TOKENS/SECOND    {state.tokens_per_second:,.0f}" if state.tokens_per_second else "TOKENS/SECOND    —",
                    f"ETA              {eta}", f"ELAPSED          {format_duration(state.elapsed)}", "",
                    f"LOSS HISTORY\n{_spark(state.loss_history)}",
                ]
                if state.gpu:
                    lines += ["", f"GPU              {state.gpu}",
